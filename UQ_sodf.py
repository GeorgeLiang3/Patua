...
class Gravity_forward():
    def __init__(self, gp_model, Reg_kernel, receivers, delta=2.):
    ...
    def calculate_slope(self):
        self.max_slope = calculate_slope_scale(
            self.kernel, rf=self.gp_model.rf)

    def create_graph(self, delta=2.):
        self.gp_model.create_tensorflow_graph(
            delta=delta, gradient=True, compute_gravity=True, max_slope=self.max_slope)
        g_center_regulargrid = GravityPreprocessing(self.kernel)
        tz_center_regulargrid = tf.constant(
            g_center_regulargrid.set_tz_kernel(), self.gp_model.tfdtype)
        self.tz = tf.constant(tz_center_regulargrid, self.gp_model.tfdtype)

    def forward(self, surface_points=None, all_properties=None,dips_position = None, dips_angles = None, sigmoid=True):
        '''
          return: final_block,final_property,block_matrix,block_mask,size,scalar_field,sfai,grav
        '''
        final_block, final_property, block_matrix, block_mask, size, scalar_field, sfai, grav = self.gp_model.compute_gravity(
            self.tz,
            surface_points=surface_points, 
            all_properties=all_properties, 
            dips_position = dips_position,
            dips_angles = dips_angles,
            kernel=self.kernel, 
...
class UQ_Patua():
  def __init__(self,gravity_forward_model,transformer,num_para_total,delta = 2.,fix_points = None, static_xy = None,fix_dips =None,args = None,Bayesargs = None,Data_Obs = None,
               num_fault_points = None,
               num_intrusion_points = None, 
               num_GT_points = None,) -> None:
    self.gravity_forward_model = gravity_forward_model
    self.gp_model = gravity_forward_model.gp_model
    self.kernel = gravity_forward_model.kernel
    self.receivers = gravity_forward_model.receivers
    self.transformer = transformer
    self.num_para_total = num_para_total
    self.fix_points = fix_points
    self.static_xy = static_xy
    self.fix_dips = fix_dips
    self.args = args
  ...
  def init_stat_model(self,Data_measurement):
    ...
    self.stat_model = Stat_model(self.gp_model,self.forward_function,self.num_para_total, tz = None, transformer = self.transformer )
  ...
  def parameter2input(self,mu,transformer = None):
    '''
    ...
    # How mu is arranges mu=[surface_points, properties, dips_angles]
    properties = mu_norm[-5-12:-12]

    faults_dips = mu_norm[-12:]
    
    # concatenate the surface points with order (intrusion, faults, strata, GT)
    sfp_z = tf.concat([self.fix_points[:self.num_intrusion_points+self.num_fault_points,2],mu_norm[:-5-12],self.fix_points[-self.num_GT_points:,2]],axis = -1)
    ...
    dips_angles = tf.concat([self.fix_dips[:4],faults_dips, self.fix_dips[4:]], axis=0)

    return sfp_xyz,properties,dips_angles
  ...
  def forward_function(self,mu,sigmoid = True):

    sfp_xyz,all_properties,dips_angles = self.parameter2input(mu,transformer= self.transformer)

    gravity = self.gravity_forward_model.forward(
                                            surface_points = sfp_xyz,
                                            all_properties = all_properties,
                                            dips_angles = dips_angles,  
                                            sigmoid = sigmoid)[-1]
...
def log_likelihood(self,mu):
    ...
    Gm_ = self.gravity_function(mu,sigmoid = True)
